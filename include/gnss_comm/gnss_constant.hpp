...
const std::map<std::string, double> type2freq = {{"G1", FREQ1},
                                                 {"G2", FREQ2},
                                                 {"G5", FREQ5},
                                                 {"R1", FREQ1_GLO},
                                                 {"R2", FREQ2_GLO},
...
struct TimeDiffMSMeasurement
{
    ...
    TimeDiffMSMeasurement()
        ...
        , tdms_carrier(0.0)
        , wavelength(0.0) {}
    TimeDiffMSMeasurement(const SatelliteData& master_sat, const SatelliteData& sat,
                          const SatelliteData& last_master_sat, const SatelliteData& last_sat,
                          double wavelength) {
        ...
        if (this->last_iSV.ion_delay != 0 && this->iSV.ion_delay != 0 &&
            this->last_master_SV.ion_delay != 0 && this->master_SV.ion_delay != 0)
        {
            this->tdms_carrier =
                (this->iSV.carrier_phase) - (this->last_iSV.carrier_phase) -
                (this->master_SV.carrier_phase) + (this->last_master_SV.carrier_phase) +
                (this->iSV.ion_delay - this->iSV.tro_delay - this->last_iSV.ion_delay +
                 this->last_iSV.tro_delay - this->master_SV.ion_delay + this->master_SV.tro_delay +
                 this->last_master_SV.ion_delay - this->last_master_SV.tro_delay + b_delta_clk_s -
                 b_delta_clk_m) /
                    wavelength;
        } else {
            this->tdms_carrier = (this->iSV.carrier_phase) - (this->last_iSV.carrier_phase) -
                                 (this->master_SV.carrier_phase) +
                                 (this->last_master_SV.carrier_phase) +
...
struct TimeDiffMSRBMeasurement
{
    ...
    TimeDiffMSRBMeasurement(SatelliteData u_master_SV_, SatelliteData u_iSV_,
        ...
        , r_iSV(r_iSV_)
        , u_last_master_SV(u_last_master_SV_)
        , u_last_iSV(u_last_iSV_)
        , r_last_master_SV(r_last_master_SV_)
        ...
        DeltaD_rs = (sat_pos_r_s_k - last_rover_pos).normalized().dot(sat_pos_r_s_k) -
                    (sat_pos_s_r_k_1 - last_rover_pos).normalized().dot(sat_pos_s_r_k_1);
        DeltaG_rs = (sat_pos_r_s_k - last_rover_pos).normalized().dot(last_rover_pos) -
                    (sat_pos_s_r_k_1 - last_rover_pos).normalized().dot(last_rover_pos);
        DeltaD_rm = (sat_pos_r_m_k - last_rover_pos).normalized().dot(sat_pos_r_m_k) -
                    (sat_pos_m_r_k_1 - last_rover_pos).normalized().dot(sat_pos_m_r_k_1);
        DeltaG_rm = (sat_pos_r_m_k - last_rover_pos).normalized().dot(last_rover_pos) -
                    (sat_pos_m_r_k_1 - last_rover_pos).normalized().dot(last_rover_pos);
        DeltaD_bm = (sat_pos_b_m_k - lase_base_pos).normalized().dot(sat_pos_b_m_k) -
                    (sat_pos_m_b_k_1 - lase_base_pos).normalized().dot(sat_pos_m_b_k_1);
        DeltaG_bm = (sat_pos_b_m_k - lase_base_pos).normalized().dot(lase_base_pos) -
                    (sat_pos_m_b_k_1 - lase_base_pos).normalized().dot(lase_base_pos);
        DeltaD_bs = (sat_pos_b_s_k - lase_base_pos).normalized().dot(sat_pos_b_s_k) -
                    (sat_pos_s_b_k_1 - lase_base_pos).normalized().dot(sat_pos_s_b_k_1);
        DeltaG_bs = (sat_pos_b_s_k - lase_base_pos).normalized().dot(lase_base_pos) -
                    (sat_pos_s_b_k_1 - lase_base_pos).normalized().dot(lase_base_pos);

        tdmsrb_carrier =
            (u_iSV.carrier_phase - u_last_iSV.carrier_phase - DeltaD_bs + DeltaG_bs) -
...
struct TimeDiffMeasurement
{
    ...
    TimeDiffMeasurement(const SatelliteData& sat, const SatelliteData& last_sat) {
        ...
        this->weight = (sin_curr_ele * sin_curr_ele / this->iSV.cp_uura +
                       ...
                       2.0;
        double delta_clk_m = (this->iSV.sat_clk - this->last_iSV.sat_clk) * LIGHT_SPEED;
        this->wavelength = sat.wavelength;
		// if (this->last_iSV.ion_delay != 0 && this->iSV.ion_delay != 0)
		// {
		// 	this->td_carrier = this->iSV.carrier_phase - this->last_iSV.carrier_phase +
		// 					   (this->iSV.ion_delay - this->iSV.tro_delay - this->last_iSV.ion_delay +
		// 						this->last_iSV.tro_delay + delta_clk_m) /
		// 						   wavelength;
		// } else {
			this->td_carrier = this->iSV.carrier_phase - this->last_iSV.carrier_phase +
