...
static double str2double(const std::string& num_str)
{
    ...
    if (D_pos != std::string::npos) tmp_str = tmp_str.replace(D_pos, 1, "e");
    return std::stod(tmp_str);
}

static GloEphemPtr rinex_line2glo_ephem(const std::vector<std::string>& ephem_lines,
                                        const uint32_t gpst_leap_seconds)
{
...
static EphemPtr rinex_line2ephem(const std::vector<std::string>& ephem_lines)
{
    ...
    if (ephem_lines[0].at(0) == 'G')
        sat_sys = SYS_GPS;
    else if (ephem_lines[0].at(0) == 'C')
        sat_sys = SYS_BDS;
    ...
    // the second line
    if (sat_sys == SYS_GPS) ephem->iode = str2double(ephem_lines[1].substr(4, 19));
...
void rinex2ephems(const std::string& rinex_filepath,
                  std::map<uint32_t, std::vector<EphemBasePtr>>& sat2ephem, double start_time,
                  double end_time)
{
    uint32_t gpst_leap_seconds = static_cast<uint32_t>(-1);
    std::ifstream ephem_file(rinex_filepath);
    if (!ephem_file.is_open())
    {
        LOG(ERROR) << "Failed to open ephemeris file: " << rinex_filepath;
        return;
    }
    ...
    // --- Body 处理与过滤部分 ---
    bool filter_enabled = (start_time > 0 && end_time > 0);   // 判断是否启用过滤

    while (std::getline(ephem_file, line))
    {
        // 1. 处理 GPS/Galileo/BDS (G/E/C)
        if (line.at(0) == 'G' || line.at(0) == 'C' || line.at(0) == 'E')
        {
            std::vector<std::string> ephem_lines;
            ephem_lines.push_back(line);
            for (size_t i = 0; i < 7; ++i)
            {
                if (!std::getline(ephem_file, line)) break;
                ephem_lines.push_back(line);
            }
            EphemPtr ephem = rinex_line2ephem(ephem_lines);
            if (!ephem || ephem->ttr.time == 0) continue;
            ...
            if (filter_enabled)
            {
                double eph_toe = time2sec(ephem->toe);   // 获取星历参考时间 TOE
                // 如果 TOE 不在指定范围内，直接丢弃
                if (eph_toe < start_time || eph_toe > end_time) continue;
            }
            // --------------------
        ...
        else if (line.at(0) == 'R')
        {
            ...
            GloEphemPtr glo_ephem = rinex_line2glo_ephem(ephem_lines, gpst_leap_seconds);
            if (!glo_ephem) continue;   // 建议加一个空指针判断

            // --- 新增：时间过滤 ---
            if (filter_enabled)
            {
                double eph_toe = time2sec(glo_ephem->toe);   // GLONASS 也有 TOE
                if (eph_toe < start_time || eph_toe > end_time) continue;
            }
            // --------------------
...
void rinex2obs(const std::string& rinex_filepath, std::vector<std::vector<ObsPtr>>& rinex_meas)
{
    ...
    {
        if (rinex_line.find("RINEX VERSION / TYPE") != std::string::npos &&
            rinex_line.find("3.04") == std::string::npos)
        {
            LOG(ERROR) << "Only RINEX 3.04 is supported for observation file";
            return;
        }
        ...
        else if (rinex_line.find("END OF HEADER") != std::string::npos)
            break;
    }
...
void rinex2iono_params(const std::string& rinex_filepath, std::vector<double>& iono_params)
{
    ...
    while (std::getline(file, line))
    {
        if (line.find("IONOSPHERIC CORR") != std::string::npos &&
            line.find("GPSA") != std::string::npos)
        {
            ...
            find_alpha = true;
                 ...
                 line.find("GPSB") != std::string::npos)
        {
...
int obs_index(const uint32_t sys, const double freq)
{
    ...
    else if (sys == SYS_GLO)
    {
        if (freq <= (FREQ1_GLO + 6 * DFRQ1_GLO))
            return 0;
        else if (freq <= (FREQ2_GLO + 6 * DFRQ2_GLO))
            return 1;
    }
    else if (sys == SYS_GAL)
    {
...
double index_freq(const ObsPtr& obs, const int band_idx, int& freq_idx)
{
    ...
    double freq_min = 0;
    double freq_max = 0;
    if (sys == SYS_GPS && band_idx == 0) { freq_min = freq_max = FREQ1; }
...
void obs2rinex(const std::string& rinex_filepath, const std::vector<std::vector<ObsPtr>>& gnss_meas)
{
    ...
    fprintf(fp,
            "%c  %3d %3s %3s %3s %3s %3s %3s %3s %3s%20s  %-20s\n",
            'G',
            8,
            "C1C",
            "L1C",
    ...
    for (int i = 0; i < (num_glo_sats <= 0 ? 1 : (num_glo_sats - 1) / 8 + 1); ++i)
    {
        ...
        for (int j = 0; j < 8; ++j)
        {
            if (i * 8 + j < num_glo_sats)
            {
                fprintf(fp,
                        " %3s %2d",
                        sat2str(glo_sat2freqo_it->first).c_str(),
                        glo_sat2freqo_it->second);
                ++glo_sat2freqo_it;
            }
            else
            {
    ...
    for (auto& meas : gnss_meas)
    {
        ...
        fprintf(fp,
                "> %04.0f %2.0f %2.0f %2.0f %2.0f%11.7f  %d%3d%21s\n",
                ep[0],
                ep[1],
                ep[2],
                ep[3],
        ...
        for (auto& obs : sorted_meas)
        {
            ...
            for (int k = 0; k < 2; ++k)
            {
                ...
                if (freq < 0) { fprintf(fp, "%16s%16s%16s%16s", "", "", "", ""); }
                else
                {
...
void ephems2nav(const std::string& nav_filepath, const std::vector<EphemPtr>& gnss_ephem)
{
    ...
    for (const auto& eph : gnss_ephem)
    {
        ...
        // 第一行：卫星 + 星历参考时间 + 钟差参数
        fprintf(fp,
                "%c%02d %4.0f %02.0f %02.0f %02.0f %02.0f%12.7f%19.12E%19.12E%19.12E\n",
                sys_char,
                eph->sat % 100,
                ep[0],
                ep[1],
