               ...
               std::vector<ObsPtr>& L1_obs, std::vector<EphemBasePtr>& L1_ephems)
{
    ...
    for (size_t i = 0; i < obs.size(); ++i)
    {
        ...
        if (sys != SYS_GPS && sys != SYS_GLO && sys != SYS_BDS && sys != SYS_GAL) continue;
             ...
             std::vector<Eigen::Vector2d>& all_sv_azel)
{
    ...
    for (uint32_t i = 0; i < num_sv; ++i)
    {
        ...
        double sagnac_term = EARTH_OMG_GPS *
                             (sv_pos(0) * rcv_state(1, 0) - sv_pos(1) * rcv_state(0, 0)) /
                             LIGHT_SPEED;
        double psr_estimated = rv2sv.norm() + sagnac_term + rcv_state(3 + sys2idx.at(this_sys)) -
                               sat_state->dt * LIGHT_SPEED + tro_delay + ion_delay +
                               sat_state->tgd * LIGHT_SPEED;
                                    ...
                                    const std::vector<double>& iono_params)
{
    ...
    while (num_iter < MAX_ITER_PVT && dx_norm > EPSILON_PVT)
    {
        ...
        for (uint32_t i = 0; i < valid_obs.size(); ++i)
        {
            ...
            if (all_sv_azel[i].y() > CUT_OFF_DEGREE / 180.0 * M_PI) good_idx.push_back(i);
        }
        ...
        LOG_IF(FATAL, num_extra_constraint >= 4)
            << "[gnss_comm::psr_pos] too many extra-clock constraints.\n";
        ...
        for (uint32_t i = 0; i < good_num; ++i)
        {
            ...
            if (this_obs->psr_std[l1_idx] > 0) weight /= (this_obs->psr_std[l1_idx] / 0.16);
        ...
        // ready for solving
        Eigen::VectorXd dx = -(good_G.transpose() * good_W * good_G).inverse() *
                             good_G.transpose() * good_W * good_b;
        dx_norm = dx.norm();
        xyzt += dx;
...
void DDdopp_res(const Eigen::Matrix<double, 3, 1>& rcv_state, const Eigen::Vector3d& rcv_ecef,
                const Eigen::Vector3d& rover_pos, const std::map<int, DDMeasurement>& dd_meas,
                Eigen::VectorXd& res, Eigen::MatrixXd& J)
{
...
void DDPsr_res(const Eigen::Matrix<double, 3, 1>& rcv_state, const Eigen::Vector3d& rcv_ecef,
               const std::map<int, DDMeasurement>& dd_meas, Eigen::VectorXd& res,
               Eigen::MatrixXd& J)
{
    const uint32_t num_sv = dd_meas.size();
    // clear output
...
void TDCPOdomYaw_res(const Eigen::Matrix<double, 3, 1>& rough_anchor_ecef,
                     const Eigen::Vector3d& ECEFPos, const Eigen::Vector3d& deltaPlocal,
                     const std::map<int, TimeDiffMSMeasurement>& tdms_meas, Eigen::VectorXd& res,
                     Eigen::MatrixXd& J)
{
    const uint32_t num_sv = tdms_meas.size();
    res = Eigen::VectorXd::Zero(num_sv);
    J = Eigen::MatrixXd::Zero(num_sv, 3);
    for (uint32_t i = 0; i < num_sv; ++i)
    {
        auto it = tdms_meas.begin();
        const TimeDiffMSMeasurement& tdms_r_meas = it->second;
        std::advance(it, i);

        double wavelength = tdms_r_meas.wavelength;
        double tdd_carrier = tdms_r_meas.tdms_carrier;
        Eigen::Vector3d sat_pos_r_m_k_1 = tdms_r_meas.last_master_SV.sat_pos;
        Eigen::Vector3d sat_pos_r_s_k_1 = tdms_r_meas.last_iSV.sat_pos;
        Eigen::Vector3d sat_pos_r_m_k = tdms_r_meas.master_SV.sat_pos;
        Eigen::Vector3d sat_pos_r_s_k = tdms_r_meas.iSV.sat_pos;

        double delta_D_s = (sat_pos_r_s_k - ECEFPos).normalized().dot(sat_pos_r_s_k) -
                           (sat_pos_r_s_k_1 - ECEFPos).normalized().dot(sat_pos_r_s_k_1);
        double delta_g_s = (sat_pos_r_s_k - ECEFPos).normalized().dot(ECEFPos) -
                           (sat_pos_r_s_k_1 - ECEFPos).normalized().dot(ECEFPos);

        double delta_D_m = (sat_pos_r_m_k - ECEFPos).normalized().dot(sat_pos_r_m_k) -
                           (sat_pos_r_m_k_1 - ECEFPos).normalized().dot(sat_pos_r_m_k_1);
        double delta_g_m = (sat_pos_r_m_k - ECEFPos).normalized().dot(ECEFPos) -
                           (sat_pos_r_m_k_1 - ECEFPos).normalized().dot(ECEFPos);


        res(i) = tdd_carrier * wavelength - delta_D_s + delta_g_s + delta_D_m - delta_g_m;
        J.block(i, 0, 1, 3) = (-(tdms_r_meas.iSV.sat_pos - ECEFPos).normalized() +
                               (tdms_r_meas.master_SV.sat_pos - ECEFPos).normalized())
                                  .transpose();
    }
}
                                     ...
                                     Eigen::Vector3d& ref_ecef)
{
    ...
    if (ref_ecef.norm() == 0)
    {
        ...
        if (psr_result.head<3>().norm() != 0) { ref_ecef = psr_result.head<3>(); }
        else
        {
            LOG(ERROR) << "[gnss_comm::dopp_vel] Unable to initialize reference position for "
                          "Doppler calculation.\n";
            return result;
        }
    }
    ...
    {
        LOG(ERROR)
            << "[gnss_comm::dopp_vel] GNSS observation not enough for velocity calculation.\n";
        return result;
    }
    ...
    while (num_iter < MAX_ITER_PVT && dx_norm > EPSILON_PVT)
    {
        ...
        for (uint32_t i = 0; i < good_num; ++i)
        {
            ...
            if (this_obs->dopp_std[l1_idx] > 0) weight /= (this_obs->dopp_std[l1_idx] / 0.256);
